...
class SocketModeClient extends eventemitter3_1.EventEmitter {
     ...
     */
    static loggerName = 'SocketModeClient';
    /**
     ...
     */
    reconnectionTimer;
    constructor({ logger = undefined, logLevel = undefined, autoReconnectEnabled = true, pingPongLoggingEnabled = false, clientPingTimeout = 5000, serverPingTimeout = 30000, appToken = '', clientOptions = {}, dispatcher = undefined, } = { appToken: '' }) {
        super();
        if (!appToken) {
        ...
        if (typeof logger !== 'undefined') {
            ...
            this.logger = logger_1.default.getLogger(SocketModeClient.loggerName, logLevel ?? logger_1.LogLevel.INFO, logger);
        }
        this.webClientOptions = clientOptions;
        if (dispatcher && this.webClientOptions.fetch === undefined) {
            this.webClientOptions.fetch = (url, init) => (0, undici_1.fetch)(url, { ...init, dispatcher: dispatcher });
        }
    ...
    async retrieveWSSURL() {
        ...
        try {
            ...
            if (error instanceof web_api_1.WebAPIPlatformError &&
                Object.values(UnrecoverableSocketModeStartError_1.UnrecoverableSocketModeStartError).includes(error.data.error)) {
                isRecoverable = false;
            }
            else if (error instanceof web_api_1.WebAPIRequestError) {
                isRecoverable = false;
            }
            else if (error instanceof web_api_1.WebAPIHTTPError) {
                isRecoverable = false;
            }
            ...
            if (this.websocket === undefined) {
                ...
                this.websocket.send(flatMessage, (error) => {
                    if (error) {
                        ...
                        return reject(new errors_1.SMWebsocketError(error));
                    }
                    return resolve();
                });
            }
